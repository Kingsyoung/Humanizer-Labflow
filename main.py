...
from filler_system import (
    get_filler_phrase,
    get_expansion_parenthetical,
    get_hedging_parenthetical,
    get_signpost_opener,
...
def enforce_length_constraint(original: str, humanized: str, max_diff: int = 3) -> str:
    ...
    # Too short: append domain-aware filler phrase
...
def _prepend_signpost(sent: str, opener: str) -> str:
    ...
    Handles capitalisation: opener ends with comma → force next word lowercase.
    """
    sent = sent.strip()
    if not sent:
        return sent
...
def final_obfuscation_layer(text: str, modification_rate: float = MODIFICATION_RATE) -> str:
    ...
    for i, sent in enumerate(sentences):
        ...
        if technique == 0 and len(words) > 10:
            ...
            # Domain-aware hedging parenthetical after an anchor noun
...
def apply_signpost_openers(text: str, rate: float = SIGNPOST_RATE) -> str:
    """Prepend domain-aware signposting openers to a subset of sentences."""
    sentences = split_sentences(text)
    processed = []
    for i, sent in enumerate(sentences):
        words = sent.split()
        if (len(words) > 5
            ...
            # Pass sentence context so opener could be domain-matched if extended
            opener = get_signpost_opener()
            sent = _prepend_signpost(sent, opener)
            sent = re.sub(r"\s+", " ", sent).strip()
            if sent and sent[-1] not in ".!?":
                sent += "."
        processed.append(sent)
...
def syntactic_burstiness_engine(sentences: List[str]) -> List[str]:
    ...
    for i, sent in enumerate(sentences):
        ...
        if pattern == 0 and len(words) < int(current_len * 1.3):
            # LONG: embed domain-aware subordinate clause
            ...
            # LONG with domain-aware expansion parenthetical
...
def local_humanize(sent: str, index: int) -> str:
    ...
    if index % 3 == 0 and len(words) > 8:
        ...
        # Use domain-aware filler for the expansion
        h = h.rstrip(".") + ", " + get_filler_phrase(h) + "."
    elif index % 3 == 2 and "," in h:
        h = h.replace(",", ";", 1)

    # Signpost opener (rate-controlled) — domain-aware
...
def humanize_with_mistral(paragraphs: List[List[str]], style: str) -> List[ParagraphData]:
    ...
    for i, para in enumerate(data.get("processed_paragraphs", [])):
        ...
        # Burstiness at paragraph level — passes sentence text for domain detection
        ...
        for j, (sent_data, h) in enumerate(zip(para_sentences, burst_sentences)):
            ...
            # Signpost openers applied before obfuscation so capitalisation is handled cleanly
